...
const Sidebar = ({ isSidebarOpen, sidebarWidth }) => {
  ...
  return (
    <div style={{ ...styles.sidebar, width: sidebarWidth, transform: isSidebarOpen ? 'translateX(0)' : `translateX(-${sidebarWidth})` }}>
      ...
      <div style={styles.footer}>
        {user ? (
          <div style={styles.userInfo}>
            <span style={styles.username}>Usuario: {user.username}</span> {/* Añadir username para depuración */}
            {user.role && <span style={styles.userRole}>Rol: ({user.role})</span>} {/* Mostrar rol para depuración */}
...
const styles = {
  ...
  username: { // Nuevo estilo para el username
    display: 'block',
    fontSize: '14px',
  ...
  loginLink: {
    ...
    margin: '0 15px', // Centrar el botón y darle más margen
    borderRadius: '4px',
    backgroundColor: '#28a745', /* Verde oscuro para el botón de iniciar sesión */
    transition: 'background-color 0.3s ease, transform 0.2s ease',
    boxShadow: '0 2px 5px rgba(0,0,0,0.2)',
  },
};
