...
class CustomUser(AbstractUser):
    ...
    role = models.CharField(
        _("role"), max_length=20, choices=ROLE_CHOICES, default="vendedor"
    )
...
class Producto(AbstractBaseModel):
    ...
    categoria = models.ForeignKey(
        Categoria, on_delete=models.CASCADE, verbose_name=_("categoría")
    )
...
class Venta(AbstractBaseModel):
    ...
    vendedor = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name=_("vendedor")
    )
    ...
    monto_total = models.DecimalField(
        _("monto total"), max_digits=10, decimal_places=2, default=0
    )
...
class DetalleVenta(AbstractBaseModel):
    ...
    producto = models.ForeignKey(
        Producto, on_delete=models.CASCADE, verbose_name=_("producto")
    )
...
class ProductoAgotado(AbstractBaseModel):
    ...
    producto = models.OneToOneField(
        Producto, on_delete=models.CASCADE, verbose_name=_("producto"), unique=True
    )
    fecha_inicio = models.DateTimeField(
        _("fecha inicio"),
        null=True,
        blank=True,
        help_text=_("Fecha cuando el producto comenzó a venderse"),
    )
    fecha_agotado = models.DateTimeField(
        _("fecha agotado"),
        default=timezone.now,
        help_text=_("Fecha cuando se agotó completamente"),
    )
    cantidad_inicial = models.IntegerField(
        _("cantidad inicial"), help_text=_("Cantidad inicial cuando comenzó a venderse")
    )
    cantidad_vendida = models.IntegerField(
        _("cantidad vendida"), help_text=_("Cantidad total vendida hasta agotarse")
    )
    tiempo_vida = models.IntegerField(
        _("tiempo vida (días)"),
        null=True,
        blank=True,
        help_text=_("Días que tardó en agotarse"),
    )
