...
class ClienteViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated]  # Temporalmente menos restrictivo para debugging
    ...
    def check_uniqueness(self, request):
        ...
        if telefono:
            ...
            if client_id:
                queryset = queryset.exclude(id=client_id)
            if queryset.exists():
                errors['telefono'] = ['Ya existe un cliente con este número de teléfono.']

        if errors:
            return Response(errors, status=status.HTTP_400_BAD_REQUEST)
        return Response({'message': 'CI y Teléfono disponibles.'}, status=status.HTTP_200_OK)
...
class VentaViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['post'], permission_classes=[IsAuthenticated, IsAdminUser | IsGerenteUser])
...
class ReporteVentasView(APIView):
    ...
    serializer_class = ReporteVentasSerializer # Añadir el serializador para la documentación
    ...
    def generar_csv(self, ventas):
        ...
        # Encabezados
        writer.writerow([
            'ID Venta', 'Cliente', 'Vendedor', 'Fecha Venta', 'Total',
...
class DashboardLowStockView(APIView):
    ...
    def get(self, request):
        ...
        low_stock_products = Producto.objects.filter(
            cantidad_disponible__lte=low_stock_threshold,
            estado=True
        ).select_related('categoria').order_by('cantidad_disponible')
...
class DashboardDailySalesView(APIView):
    ...
    def get(self, request):
        ...
        yesterday_sales = Venta.objects.filter(
...
class DashboardSalesTrendView(APIView):
    ...
    def get(self, request):
        ...
        for i in range(30):
            ...
            day_sales = Venta.objects.filter(
                fecha_venta__date__gte=current_date,
                fecha_venta__date__lt=next_date
            ).aggregate(total=Sum('total'))['total'] or 0
    ...
    def get(self, request):
        ...
        for categoria in categories:
            categories_data.append({
                ...
                'total': categoria.total_products or 0
            })
...
class DashboardRecentSalesView(APIView):
    ...
    def get(self, request):
        recent_sales = Venta.objects.select_related(
            'cliente', 'vendedor'
        ).prefetch_related('detalles').order_by('-fecha_venta')[:10]
...
class ProductoAgotadoViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def generar_reporte_pdf(self, request):
        ...
        # Título
        titulo = Paragraph("Reporte de Productos Agotados y Tendencias - Joyería Trebol", styles['Title'])
        elements.append(titulo)
        elements.append(Spacer(1, 20))

        # Resumen estadístico
        total_productos = productos_agotados.count()
        tiempo_promedio = productos_agotados.aggregate(avg=Avg('tiempo_vida'))['avg'] or 0
        ...
        filename = f"reporte_productos_agotados_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        response['Content-Disposition'] = f'attachment; filename="{filename}"'
