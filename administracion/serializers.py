    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            venta = Venta.objects.create(
                cliente=cliente, vendedor=self.context["request"].user
            )
            ...
            for detalle_data in detalles_data:
                try:
                    ...
                    raise serializers.ValidationError(
                        {
                            "detalles": [
                                f"Producto con ID {detalle_data['producto_id']} no encontrado."
                            ]
                        }
                    )
                ...
                if producto.cantidad_disponible == 0:
                    ...
                    ProductoAgotado.objects.create(
                        ...
                        + cantidad,  # Cantidad antes de esta venta
...
class VentaSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            # Crear la venta
            venta = Venta.objects.create(
                cliente=cliente, vendedor=self.context["request"].user, **validated_data
            )
...
class ReporteVentasSerializer(serializers.Serializer):
    ...
    fecha_inicio = serializers.DateField(
        required=False, help_text="Fecha de inicio (YYYY-MM-DD)"
    )
    fecha_fin = serializers.DateField(
        required=False, help_text="Fecha de fin (YYYY-MM-DD)"
    )
    formato = serializers.ChoiceField(
        choices=["csv", "pdf"],
        default="csv",
        required=False,
        help_text="Formato del reporte (csv o pdf)",
    )
